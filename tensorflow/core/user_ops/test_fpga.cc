...
template <typename T>
class TestFpgaOp : public OpKernel {
public:
    explicit TestFpgaOp(OpKernelConstruction* ctx) : OpKernel(ctx) {}
...
#define REGISTER_KERNEL(type)                                       \
    REGISTER_KERNEL_BUILDER(                                          \
        Name("TestFpga").Device(DEVICE_FPGA).TypeConstraint<type>("T"), \
        TestFpgaOp<type>)
