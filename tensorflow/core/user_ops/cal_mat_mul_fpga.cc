...
template <typename T>
class CalMatMulFpgaOp : public OpKernel {
public:
    explicit CalMatMulFpgaOp(OpKernelConstruction* ctx) : OpKernel(ctx) {}
    ...
    void Compute(OpKernelContext* ctx) override {
        ...
        int send_a_length = a_shape.dim_size(0)*a_shape.dim_size(1);
		int send_b_length = b_shape.dim_size(0)*b_shape.dim_size(1);
		
        int *send_a = new int[send_a_length];
        int *send_b = new int[send_b_length];

        int **recv_c = new int*[output_shape.dim_size(0)];
		for(int i=0; i<output_shape.dim_size(0); i++){
			recv_c[i] = new int[output_shape.dim_size(1)];
		}
		
		for(int i=0; i<a_shape.dim_size(0); i++){
            for(int j=0; j<a_shape.dim_size(1); j++){
                send_a[i*a_shape.dim_size(1)+j] = input0(i, j);
            }
        } 
		for(int i=0; i<b_shape.dim_size(0); i++){
            for(int j=0; j<b_shape.dim_size(1); j++){
                send_b[i*b_shape.dim_size(1)+j] = input1(i, j);
            }
        } 
		
		/* for(int i=0; i<send_a_length; i++){
			cout<<send_a[i]<<" "; 
		}
		cout<<endl;
		cout<<"======================"<<endl;
		for(int i=0; i<send_b_length; i++){
			cout<<send_b[i]<<" "; 
		}
		cout<<endl; */
        sendToFpga(send_a, a_shape.dim_size(0), a_shape.dim_size(1), send_b, b_shape.dim_size(0), b_shape.dim_size(1));
		recvFromFpga(recv_c, output_shape.dim_size(0), output_shape.dim_size(1));
        
		for(int i=0; i<output_shape.dim_size(0); i++){
            for(int j=0; j<output_shape.dim_size(1); j++){
                output(i, j) = recv_c[i][j];
            }
        } 
...
#define REGISTER_KERNEL(type)                                       \
    REGISTER_KERNEL_BUILDER(                                          \
        Name("CalMatMulFpga").Device(DEVICE_CPU).TypeConstraint<type>("T"), \
        CalMatMulFpgaOp<type>)
